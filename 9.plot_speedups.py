...
for i in range(len(grid_configs)):
    ...
    ax.plot(df_n['NUM_THREADS'], df_n['Amdahl Speedup'], marker='o', label='Amdahl Speedup')
    ax.plot(df_n['NUM_THREADS'], df_n['Gustafson Speedup'], marker='o', label='Gustafson Speedup')
...
# Save the plot
plt.savefig('./Images/amdahl_vs_threads.png', dpi=300)
